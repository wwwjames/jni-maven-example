...
JNIEXPORT jstring JNICALL Java_com_jni_example_JniWrapper_getString(JNIEnv *env, jobject thiz) {
    ...
    jmethodID callback_with_object = (*env)->GetMethodID(env, cls_foo, "callbackObject", "(Ljava/util/List;)V");
    jmethodID callback_static = (*env)->GetStaticMethodID(env, cls_foo, "callbackStatic", "(Ljava/lang/String;)V");
    ...
    (*env)->CallStaticVoidMethod(env, thiz, callback_static, jStringStatic);
    ...
    (*env)->CallObjectMethod(env, arraylist, addMethod, jStringPayload);
    (*env)->CallVoidMethod(env, thiz, callback_with_object, arraylist);
